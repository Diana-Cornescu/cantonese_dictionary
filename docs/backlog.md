...
| **Filter by tag** | On the home list and in the photo gallery. Starred / hard filters are done on both (1.4.0); a tag filter needs a picker rather than one more toggle, since there can be any number of tags. The Tags screen already lists a tag's characters, so this is about filtering in place rather than navigating away. |  You, 2026-09-20 (suggested) |
