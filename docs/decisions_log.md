...
- **Home list filters:** ⭐ and 🔥 buttons to the **right of the search box**, filtering the list rather than flagging anything.
  ...
  - **Showing that a filter is on** is the whole point of the feature. The first attempt gave an active button three signals at once — filled coloured icon, `primaryContainer` background, primary-coloured border. **Revised 2026-09-21 to the icon alone**: the background and border were loud, changed appearance with every color theme, and competed with the gold and red that carry the actual meaning. The outline is now a fixed light grey (`AppColors.inactive`) in every theme, and the icon does the talking — grey outline when off, gold star or red fire when on.
...
## 2026-09-21 — Photos became the third face of the character box

- **Request:** a **Photos** button alongside Typed and Handwritten, showing a carousel of the photos linked to the character.
- **The old Photos section is gone.** It was a strip of 88 px thumbnails near the bottom of the screen. Keeping both would have meant two ways to reach the same photos, and the carousel is the better one — bigger, and in the place you already look. Its "add" tile became an **Add photo** button under the carousel, so nothing was lost. The screen is shorter for it.
- **Arrows, not dots** (asked for): a round outlined arrow sits **outside** the box on each side. It does two jobs — says "this swipes" to anyone who wouldn't think to try, and moves the carousel for anyone who'd rather tap. They grey out at each end instead of wrapping around, so the ends of the run are obvious without counting dots.
- **The arrows only exist in the photos view**, which is why they're a `Row` wrapped around this face only: the typed and handwritten faces keep exactly the layout they had, rather than being re-parented and re-measured for a feature they don't use.
- `_showHandwritten` (a bool) became `_CharacterView` (an enum). Two states fit in a bool; three don't.
- **After adding a photo the carousel jumps to it.** `photosFor` is newest-first so it lands at index 0, but the jump has to wait for a post-frame callback: the `PageView` must rebuild with the new photo in it before the controller can be moved onto it. Without that the viewport keeps its old offset and you're looking at a different photo than the one you just took.
- **The page index is clamped on every build.** A photo can be deleted from its own screen while this index points past the end of the list.
- **Filter tooltips stopped changing with state** (2026-09-21). They used to append " (filtering)" while on, so two buttons side by side described themselves differently depending on which was active. The icon already says which is on; the tooltip just names the filter.
- **`AppColors.silverGrey`** added for this, with the alias `inactive`. Per the palette file's own rule, no raw hex at the call site.
- **The character box's toggles lost their themed fill too** (2026-09-21). Typed / Handwritten / Photos and Favorite / Hard all used `_selectedButtonStyle`, which filled the selected one with `primaryContainer` — the same objection as the filter buttons, in a second place. **Favorite / Hard** now share one look with the filter buttons, in fixed greys: unselected is light grey throughout — 1px outline, icon and label (`AppColors.inactive`); selected is a **black 1.5px outline**, a **pale grey background** (`AppColors.selectedOutline` / `selectedFill`), the icon in its own color — gold star, red fire — and a dark grey label. Three signals, none theme-colored, so the only color in the button is the one that means something. These are the same two flags the list rows and the filter buttons show, so they look the same wherever they appear.
