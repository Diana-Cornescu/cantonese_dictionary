...
| 5 | Display | Character screen (in the Tags section), and on **Flashcards and Write**: the card's language as name + emblem, centred directly under the card / writing box, same spot on both screens, black / white by mode. The line keeps its height when there's no language, so Go to character screen under it never moves. The Language checkboxes in both … panels carry the emblems too. The list rows are unchanged. | Chosen 2026-09-26. |
